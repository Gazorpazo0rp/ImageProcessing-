...
t6=15 #ORB feature extractor edge threshold
MIN_MATCH_COUNT=1
armWidth=80 
armMask=[[],[]]
...
def Homography(armKps,matches,armGray,whichArm):
    
    if whichArm ==1:
        liveKps=leftWindowKps
        theWindow=window[1]
    else :
        liveKps=rightWindowKps
        theWindow=window[0]
    ...
    if len(matches)>=MIN_MATCH_COUNT: 
        ...
        #theWindow = cv2.polylines(theWindow,[np.int32(dst)],True,255,3, cv2.LINE_AA)
    ...
    liveWithMatches =cv2.drawMatches(theWindow,liveKps,armGray,armKps,matches,None,**draw_params)
...
def main(): 
    #
    global liveDes,liveKps,liveFrame,liveGray,leftArmKps,rightArmKps,window,leftWindowKps,leftWindowDes,rightWindowKps,rightWindowDes
    setup()
    detectArm(0)
    showDetected(person)
    ...
    while True:
        ...
        if liveRet:
            ...
            rightWindowKps,rightWindowDes = OrbFeaturesExtractor(window[0])
        
            if leftWindowDes is None or rightWindowDes is None:
                print("one of the windows descriptor list is empty")
                continue
            #create matcher obj
            ...
            leftArmMatches=BFMatcher(leftArmKps,leftArmDes,leftWindowDes)
            rightArmMatches=BFMatcher(rightArmKps,rightArmDes,rightWindowDes)
            ...
            for i in range(len(leftArmMatches)):
                sumx+=leftWindowKps[leftArmMatches[i].queryIdx].pt[0]
                sumy+=leftWindowKps[leftArmMatches[i].queryIdx].pt[1]
            globalAvgX=sumx//len(leftArmMatches)
            globalAvgX=int(globalAvgX)
            globalAvgY=sumy//len(leftArmMatches)
            globalAvgY=int(globalAvgY)
            cv2.circle(window[1],(globalAvgX,globalAvgY),3,255,5)
            
            for i in range(len(rightArmMatches)):
                sumx+=rightWindowKps[rightArmMatches[i].queryIdx].pt[0]
                sumy+=rightWindowKps[rightArmMatches[i].queryIdx].pt[1]
            globalAvgX=sumx//len(rightArmMatches)
            globalAvgX=int(globalAvgX)
            globalAvgY=sumy//len(rightArmMatches)
            globalAvgY=int(globalAvgY)
            cv2.circle(window[0],(globalAvgX,globalAvgY),3,255,5)
            
            
            success,finalArmLeft=Homography(leftArmKps,leftArmMatches,armMask[1],1)
            if success:
                window2 = cv2.namedWindow("LeftArmMatching", cv2.WINDOW_NORMAL)
                cv2.resizeWindow("LeftArmMatching", 700,500)
                cv2.imshow("LeftArmMatching", np.fliplr(finalArmLeft))
                
            success,finalArmRight=Homography(rightArmKps,rightArmMatches,armMask[0],0)
            if success:
                window2 = cv2.namedWindow("RightArmMatching", cv2.WINDOW_NORMAL)
                cv2.resizeWindow("RightArmMatching", 700,500)
                cv2.imshow("RightArmMatching", np.fliplr(finalArmRight))
            
            '''
            if len(leftArmMatches)>=MIN_MATCH_COUNT: 
             
                src_pts = np.float32([leftWindowKps[m.queryIdx].pt for m in leftArmMatches ]).reshape(-1,1,2)
                
                dst_pts = np.float32([leftArmKps[m.trainIdx].pt for m in leftArmMatches ]).reshape(-1,1,2)
                ...
                window[1] = cv2.polylines(window[1],[np.int32(dst)],True,255,3, cv2.LINE_AA)
        
            else:
                print ("Not enough matches are found - %d/%d" % (len(leftArmMatches),MIN_MATCH_COUNT))
                matchesMaskLeft = None
            draw_params = dict(matchColor = (0,255,0), # draw matches in green color
                   singlePointColor = None,
                   matchesMask = matchesMaskLeft, # draw only inliers
            ...
            liveWithMatches =cv2.drawMatches(window[1],leftWindowKps,armMask[1],leftArmKps,leftArmMatches,None,**draw_params)
            cv2.imshow("LeftArmMatching", np.fliplr(liveWithMatches))'''
            #for i in range(len(leftArmMatches)):
